...
from models.cnn_lstm.conv_lstm import ConvLSTM, ConvLSTMModel
...
if __name__ == "__main__":
    ...
    search_space = {
        "input_dim": 3,
        "hidden_dim": tune.choice([2**i for i in range(5, 9)]),
        "kernel_size": tune.choice([3, 5, 7, 9, 11]),
        "stride": 1,
        "padding": "same",
        "num_layers": tune.choice([i for i in range(1, 8)]),
        "batch_first": True,
        "bias": tune.choice([True, False]),
        "return_all_layers": False,
        "lr": tune.loguniform(1e-4, 1e-3),
        "batch_size": tune.choice([2**i for i in range(1, 6)]),
        "steps": tune.choice([2**i for i in range(5, 8)]),
    }
