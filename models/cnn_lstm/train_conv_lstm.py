...
from models.cnn_lstm.conv_lstm import ConvLSTMModel
...
def train_model(
    ...
    Returns:
        None
    """
    writer = SummaryWriter(f"runs/cnn_conv_lstm_{timestamp}")
...
if __name__ == "__main__":
    ...
    search_space = {
        "input_dim": 3,
        "hidden_dim": tune.choice([2**i for i in range(5, 7)]),
        "kernel_size": tune.choice([(3, 3), (5, 5), (7, 7), (9, 9), (11, 11)]),
        "stride": 1,
        "padding": "same",
        "num_layers": tune.choice([i for i in range(1, 5)]),
        "batch_first": True,
        "bias": tune.choice([True, False]),
        "return_all_layers": False,
        "lr": tune.loguniform(1e-4, 1e-3),
        "batch_size": tune.choice([2**i for i in range(1, 4)]),
        "steps": tune.choice([2**i for i in range(5, 7)]),
    ...
    best_trained_model = ConvLSTMModel(
        input_dim=3,
    ...
    for value in dfs.values():
        value.to_csv(f"{main_folder_path}/models/cnn_lstm/{MODEL_NAME}_best_result.csv", index=False)
